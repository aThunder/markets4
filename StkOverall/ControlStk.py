...
'''This program will call each of the following programs:
    1. stkList.py: Prompts user to specify list of stock symbols
                      and returns list to ControlStks.py
    2.setCSV.py: Checks to make sure each symbol is valid
                        and creates a CSV file for each valid symbol
                        and returns CSV files to ControlStks.py
    3. stkSQLP.py: ControlStks.py prompts for whether to populate
    ...
    Iteration Overview:
    1. stkList.py : N/A
    2. stkCSV: Processes one symbol at a time as each one is passed from ControlStksOnly.py
    3. stkSQLP.py: Receives a List from ControlStksOnly.py and iterates through it
